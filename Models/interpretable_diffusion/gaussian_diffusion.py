...
from Models.interpretable_diffusion.DiM import DiM
from Models.interpretable_diffusion.model_utils import default, identity, extract
from eval_utils import BMMD, cross_correlation_distribution
...
class DiM_TS(nn.Module):
    def __init__(
        ...
        if model_type=='DiM':
            trans_mx0 = None
            if trans_mx is not None:
                s_list = torch.from_numpy(np.load(trans_mx))
                trans_mx0 = torch.zeros((s_list.shape[0], s_list.shape[0]))
    ...
    def _train_loss(self, x_start, t, target=None, noise=None):
        ...
        if self.use_ff:
            ...
            train_loss +=  self.ff_weight * fourier_loss * self.l_loss
        
        train_loss = reduce(train_loss, 'b ... -> b (...)', 'mean')
        train_loss = train_loss * extract(self.loss_weight, t, train_loss.shape)
        # MMD loss
        if self.mmd_alpha!=0:
            target_dist = (cross_correlation_distribution(target).unsqueeze(-1).permute(1, 0, 2))
            pred_dist = (cross_correlation_distribution(model_out).unsqueeze(-1).permute(1, 0, 2))
            mmd_loss = BMMD(target_dist, pred_dist, "rbf").mean()
            return train_loss.mean() + self.mmd_alpha*mmd_loss
    ...
    def forward(self, x, **kwargs):
        ...
        if self.sample_type=='batch':
            ...
            p = w / np.sum(w)
            t0 = np.random.choice(len(p), size=1, p=p)
            indices_np = np.array([t0[0] for i in range(b)])
            t = torch.from_numpy(indices_np).long().to(device)
